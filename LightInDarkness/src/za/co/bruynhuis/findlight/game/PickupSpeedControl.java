 ...
 * @author NideBruyn
 */
public class PickupSpeedControl extends AbstractControl implements AnimationListener {
    ...
    public PickupSpeedControl(Game game, AnimatedSprite sprite, int amount) {
        this.game = game;
        this.amount = amount;
        this.sprite = sprite;
    }
    ...
    public void doPickup() {
        if (!pickedUp) {
            spatial.setCullHint(Spatial.CullHint.Always);
            pickedUp = true;
            ((Player) game.getPlayer()).addSpeed();
            sprite.play("done", true, false, false);
